...
const statusConfig: Record<string, { color: string; icon: React.ReactNode }> = {
  ...
  Pending:  { color: 'text-amber-600', icon: <Clock        className="w-3.5 h-3.5" /> },
...
export default function DistributorMgmt() {
  ...
  const filtered = distributors.filter(d => {
    const matchSearch = d.name.toLowerCase().includes(search.toLowerCase()) ||
                        d.arn.toLowerCase().includes(search.toLowerCase());
                ...
                return (
                  <tr key={d.id} className="group hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-slate-800 group-hover:text-blue-600 transition-colors">{d.name}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{d.email}</div>
                    </td>
                    <td className="px-6 py-4 text-sm font-mono text-slate-600">{d.arn}</td>
                    <td className="px-6 py-4 font-mono font-semibold text-slate-800">{d.aum}</td>
                    <td className="px-6 py-4 text-sm font-medium text-slate-700">{d.investors.toLocaleString()}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 text-xs font-bold rounded-md border ${tierConfig[d.tier]}`}>
                        {d.tier}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-500">{d.onboarded}</td>
